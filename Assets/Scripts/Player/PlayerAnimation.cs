...
public class PlayerAnimation : MonoBehaviour
{
    ...
    private Vector2 _velocityBeforePhysiscsUpdate;
    ...
    {
        _velocityBeforePhysiscsUpdate = _rigidBody.velocity;
    }

    private void OnCollisionEnter2D(Collision2D other)
    {
        if (_velocityBeforePhysiscsUpdate.y < _yLandVelocityCheck)
        {
            PlayPoofDustVFX();
    ...
    private void ApplyTilt()
    {
        ...
        _cowboyHatTransform.rotation = Quaternion.Lerp(currentHatRotation, targetHatRotation, _tiltSpeed * Time.deltaTime * _cowboyHatTiltModifier);
